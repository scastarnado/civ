 ...
 */
import { CHUNK_SIZE, ResourceNodeType, TileType, } from '@/core/types';
/**
...
export class ChunkGenerator {
     ...
     */
    backfillResourceNodes(chunk) {
        return {
            ...chunk,
            tiles: chunk.tiles.map((tile) => ({
                ...tile,
                resourceNode: tile.resourceNode ?? this.pickResourceNode(tile.type, tile.x, tile.y),
            })),
        };
    }
    /**
     ...
     */
    pickResourceNode(tileType, x, y) {
        const roll = this.hash01(x, y, this.worldSeed ^ 0x5f3759df);
        if (tileType === TileType.GRASSLAND) {
            if (roll < 0.02)
                return ResourceNodeType.WHEAT;
            if (roll < 0.03)
                return ResourceNodeType.HORSES;
            if (roll < 0.035)
                return ResourceNodeType.GOLD;
        }
        if (tileType === TileType.FOREST) {
            if (roll < 0.03)
                return ResourceNodeType.DEER;
        }
        if (tileType === TileType.MOUNTAIN) {
            if (roll < 0.04)
                return ResourceNodeType.IRON;
            if (roll < 0.048)
                return ResourceNodeType.GOLD;
        }
        return undefined;
...
export class MapCache {
     ...
     */
    getChunksInRegion(cx1, cy1, cx2, cy2) {
        const chunks = [];
...
//# sourceMappingURL=ChunkSystem.js.map