...
export class GameEngine {
    ...
    getStartingPositionForPlayer(playerId) {
        ...
        for (let attempt = 0; attempt < 256; attempt++) {
            ...
            if (tile.type === TileType.WATER || tile.type === TileType.MOUNTAIN)
                continue;
            const occupied = this.gameState.players.some((existingPlayer) => existingPlayer.cities.some((city) => city.x === x && city.y === y));
            if (!occupied) {
                return { x, y };
            }
        }
