...
import { db } from './db';
import type { CivilizationRole, GovernanceType } from './types';
...
async function getGovernanceType(
	civilizationId: number,
): Promise<GovernanceType | null> {
	const result = await db.query(
		`SELECT governance_type FROM civilizations WHERE id = $1`,
		[civilizationId],
	);
	if (result.rowCount === 0) return null;
	return result.rows[0].governance_type as GovernanceType;
}

async function getMemberRole(
	civilizationId: number,
	userId: number,
): Promise<CivilizationRole | null> {
	const result = await db.query(
		`SELECT role FROM civilization_members WHERE civilization_id = $1 AND user_id = $2`,
		[civilizationId, userId],
	);
	if (result.rowCount === 0) return null;
	return result.rows[0].role as CivilizationRole;
}

function canManageJoinRequests(
	role: CivilizationRole,
...
async function getUserPrimaryCivilization(
	userId: number,
	client: Queryable = db,
): Promise<CivilizationSummary | null> {
	const result = await client.query(
		`SELECT c.id, c.name, c.governance_type, m.role
...
function makePersonalCivName(username: string, attempt: number): string {
	const base = `${username}'s Civilization`;
	if (attempt === 0) return base;
	return `${base} ${attempt + 1}`;
}

async function createPersonalCivilization(
	userId: number,
	username: string,
	client: PoolClient,
): Promise<CivilizationSummary> {
	for (let attempt = 0; attempt < 20; attempt += 1) {
		const civName = makePersonalCivName(username, attempt);
		try {
			const created = await client.query(
				`INSERT INTO civilizations (name, description, governance_type, created_by)
				 VALUES ($1, $2, 'community', $3)
				 RETURNING id, name, governance_type`,
				[civName, `Personal civilization for ${username}`, userId],
			);
			const civRow = created.rows[0] as {
				id: string;
				name: string;
				governance_type: GovernanceType;
			};
			await client.query(
				`INSERT INTO civilization_members (civilization_id, user_id, role)
				 VALUES ($1, $2, 'ruler')`,
				[Number(civRow.id), userId],
			);
			return {
				id: Number(civRow.id),
				name: civRow.name,
...
async function ensureUserHasCivilization(
	userId: number,
	username: string,
): Promise<CivilizationSummary> {
...
app.post('/api/auth/register', async (req, res) => {
	...
	try {
		...
		const civilization = await ensureUserHasCivilization(
			Number(row.id),
			row.username,
		);
...
app.post('/api/auth/forgot-password', async (req, res) => {
	...
	const userResult = await db.query(
		`SELECT id, username FROM users WHERE username = $1 OR email = $1 LIMIT 1`,
		[identifier],
	);
	...
	const userRow = userResult.rows[0] as { id: string; username: string };
...
app.post('/api/civs', requireAuth, async (req, res) => {
	...
	const governanceType = String(
		req.body?.governanceType || '',
	).trim() as GovernanceType;
	if (
		!name ||
	...
	const created = await db.query(
		`INSERT INTO civilizations (name, description, governance_type, created_by)
		 VALUES ($1, $2, $3, $4)
		 RETURNING id, name, description, governance_type`,
		[name, description, governanceType, user.id],
	);
	const civ = created.rows[0] as {
		id: string;
		name: string;
		description: string;
		governance_type: GovernanceType;
	...
	await db.query(
		`INSERT INTO civilization_members (civilization_id, user_id, role) VALUES ($1, $2, 'ruler')`,
		[Number(civ.id), user.id],
	);
	res.json({
		ok: true,
		civilization: {
...
app.get('/api/civs/:id', requireAuth, async (req, res) => {
	...
	const civResult = await db.query(
		`SELECT id, name, description, governance_type, created_at FROM civilizations WHERE id = $1`,
		[civilizationId],
	);
	...
	const pendingRequestsResult = await db.query(
		`SELECT r.id, r.requester_id, u.username AS requester_username, r.created_at
	...
	res.json({
		...
		civilization: {
			...
			members: (membersResult.rows as MemberRow[]).map((row) => ({
			...
			pendingJoinRequests: (
				pendingRequestsResult.rows as PendingJoinRequestRow[]
			).map((row) => ({
				id: Number(row.id),
...
app.get('/api/civs/my', requireAuth, async (req, res) => {
	const user = req.authUser!;
	const result = await db.query(
		`SELECT c.id, c.name, c.description, c.governance_type, m.role
		 FROM civilization_members m
		 JOIN civilizations c ON c.id = m.civilization_id
		 WHERE m.user_id = $1`,
		[user.id],
	);
	type MyCivilizationRow = {
		id: string;
		name: string;
		description: string;
		governance_type: GovernanceType;
	...
	res.json({
		ok: true,
		civilizations: (result.rows as MyCivilizationRow[]).map((row) => ({
			id: Number(row.id),
			name: row.name,
...
app.post('/api/civs/:id/join-request', requireAuth, async (req, res) => {
	...
	const alreadyMember = await db.query(
		`SELECT 1 FROM civilization_members WHERE civilization_id = $1 AND user_id = $2`,
		[civilizationId, user.id],
	);
	...
	try {
		await db.query(
			`INSERT INTO civilization_join_requests (civilization_id, requester_id, status)
			 VALUES ($1, $2, 'pending')`,
			[civilizationId, user.id],
		);
		res.json({ ok: true });
...
app.post(
	'/api/civs/:id/join-requests/:requestId/decide',
	requireAuth,
	async (req, res) => {
		const user = req.authUser!;
		const civilizationId = Number(req.params.id);
		...
		const requestResult = await db.query(
			`SELECT requester_id, status FROM civilization_join_requests WHERE id = $1 AND civilization_id = $2`,
			[requestId, civilizationId],
		);
		...
		await db.query(
			`UPDATE civilization_join_requests
		...
		if (approve) {
			await db.query(
				`INSERT INTO civilization_members (civilization_id, user_id, role)
			 VALUES ($1, $2, 'citizen')
			 ON CONFLICT (civilization_id, user_id) DO NOTHING`,
				[civilizationId, Number(requestRow.requester_id)],
			);
		}
...
app.post(
	'/api/civs/:id/members/:memberId/role',
	requireAuth,
	async (req, res) => {
		const user = req.authUser!;
		const civilizationId = Number(req.params.id);
		const memberId = Number(req.params.memberId);
		const nextRole = String(req.body?.role || '').trim() as CivilizationRole;
		...
		await db.query(
			`UPDATE civilization_members SET role = $1 WHERE civilization_id = $2 AND user_id = $3`,
			[nextRole, civilizationId, memberId],
		);
		res.json({ ok: true });
	},
);
